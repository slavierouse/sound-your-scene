    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="space-y-8">
          ...
          {/* HR@K by Hit Component and Image Presence Row */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              ...
              <Plot
                data={Object.entries(data?.segmented_charts?.by_image_presence || {}).map(([imageType, imageData]) => ({
                  x: imageData.map(d => d.k),
                  y: imageData.map(d => d.hr_at_k),
                  type: 'scatter',
                  mode: 'lines+markers',
                  ...
                  error_y: {
                    type: 'data',
                    array: imageData.map(d => d.ci_upper - d.hr_at_k),
                    arrayminus: imageData.map(d => d.hr_at_k - d.ci_lower),
                    visible: true,
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                ...
                <Plot
                  data={Object.entries(data?.analysis_tables?.conversation_analysis?.turns_by_model || {}).map(([model, modelData], index) => ({
                    x: modelData.map(d => d.turns),
                    y: modelData.map(d => d.percentage),
                    type: 'scatter',
                    mode: 'lines',
                    name: model,
                    line: { color: `hsl(${index * 120}, 70%, 50%)` }
          ...
          {/* Genre and Filters Analysis Row */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {(data?.analysis_tables?.genre_usage || []).map((row, index) => (
                      <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{row.genre}</td>
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{row.usage_count}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            <div className="bg-white p-6 rounded-lg shadow">
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Filter Field</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {(data?.analysis_tables?.filters_analysis?.top_filters || []).map((row, index) => (
                      <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        <td className="px-4 py-3 whitespace-nowrap text-sm font-medium text-gray-900">{row.field}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">
                          <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${{
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{row.usage_count}</td>
                        <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-500">{row.avg_value || 'N/A'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">First Search</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Latest Search</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">HR@10</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {(data?.analysis_tables?.leaderboards?.top_users || []).map((row, index) => (
                      <tr key={index} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-mono text-gray-900">{row.user || 'Unknown'}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{row.session_count}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{row.search_count}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{row.search_job_count}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{row.first_search ? new Date(row.first_search).toLocaleString() : 'N/A'}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{row.latest_search ? new Date(row.latest_search).toLocaleString() : 'N/A'}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{row.hr_at_10}%</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
